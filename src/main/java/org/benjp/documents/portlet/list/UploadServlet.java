...
public class UploadServlet extends HttpServlet {
	...
	private static final Log log = ExoLogger.getLogger(UploadServlet.class.getName());
	@Override
	...
	protected void doPost(HttpServletRequest request,
		...
		String name = (isPrivateContext) ? request.getRemoteUser() : request
				.getHeader("app-space");
		String uuid = null;
			...
			for (FileItem item : items) {
				if (item.isFormField()) {
					...
					if ("app-space".equals(fieldName)) {
						name = (isPrivateContext) ? request.getRemoteUser()
								: item.getString();
					}
				...
				if (item.getFieldName().equals("pic")) {
					...
					if (uuid != null) {
						...
						response.getWriter()
								.write("<div style='background-color:#ffa; padding:20px'>File has been uploaded successfully!</div>");
						return;
					} else {
						...
						response.setCharacterEncoding("UTF-8");
						response.getWriter()
								.write("{\"status\":\"File has been uploaded successfully!\"}");
						return;
					}
				}
			}
	...
	private void storeFile(String path, FileItem item, String name,
			boolean isPrivateContext, HttpServletRequest request) {
		storeFile(path, item, name, isPrivateContext, null, request);
	}

	private void storeFile(String path, FileItem item, String name,
			boolean isPrivateContext, String uuid, HttpServletRequest request) {
		...
		try {
			...
			if (isPrivateContext) {
				Node userNode = nodeHierarchyCreator.getUserNode(
			...
			if (!docNode.hasNode(filename)
				...
				fileNode.setProperty("exo:lastModifier",
